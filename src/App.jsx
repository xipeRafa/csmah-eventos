...
localStorage.setItem('urlArr', JSON.stringify(linksState))
    ...
    const postCollection = collection(firestoreDB, "CSMA-eventos");

    const postFile = (selectedFile) => {
       
        selectedFile.map((el, i)=>{

            const filesFolderRef = ref(storageDocs,`Files-eventos/${Date.now()}/${el.name}`);
    ...
    console.log(CSMAState)
    const [getArr, setGetArr] = useState(false);


    useEffect(() => {
        const data = collection(firestoreDB, 'CSMA-eventos')

        getDocs(data)
            .then((resp) => {
                let r = resp.docs.map((doc) => ({ ...doc.data() }))
                setCSMAState(r)
    ...
    return (
        <div className="containerApp">
            ...
            <h1>
                <span onClick={() => setCount(0)}>PROXIMOS</span>
...
{/*
            ...
            <div className='galeryContainer'>
                {CSMAState.sort((a, b) => b.duration - a.duration).slice(0,8).map((el, i) => (
